...
class ScreenerService:
    ...
    def run(self, req: ScreenerRequest, default_as_of=None) -> list[ScreenerRow]:
        as_of = req.as_of
        ...
        pin = as_of or default_as_of
        if pin is not None and (avg_n != 14 or atr_n != 14 or rel_n != 14 or window != 5):
            log.info("screener_recompute_derived", avg_n=avg_n, atr_n=atr_n, rel_n=rel_n, window=window)
            self._wh.rewrite_derived(pin, avg_n, atr_n, rel_n, window)
        rows = self._wh.run_screener(req, as_of)
        log.info("screener_done", as_of=str(as_of) if as_of else "latest", n=len(rows), preset=req.preset)
        return rows
